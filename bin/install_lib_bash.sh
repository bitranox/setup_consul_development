...
function update_lib_bash_if_exist {
    ...
    if [[ -d "${my_dir}/lib_bash" ]]; then
        cd lib_bash
        local git_remote_hash=$(git --no-pager ls-remote | grep HEAD | awk '{print $1;}' > /dev/null 2>&1 )
        local git_local_hash=$(git --no-pager log --decorate=short --pretty=oneline -n1 | grep HEAD | awk '{print $1;}' > /dev/null 2>&1 )
